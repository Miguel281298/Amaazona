<?php
    ...
    foreach ($productos as $index => $value) {
        ...
        "UPDATE Productos SET Stock = 0 WHERE ID_Producto = $value" :
    ...
    if (!$direccion) {
        echo '<script> alert("Selecciona una direccion de entrega");</script>';
        echo '<script> window.location.href = "carrito.php";</script>';
        exit();
    }
    ...
    if ($result->num_rows > 0) {
        ...
        $numeroInterior = isset($direccion['Num_Interior']) ? $direccion['Num_Interior'] : null;
